...
const Reports: React.FC = () => {
  ...
  const last7Days = [...Array(7)].map((_, i) => {
    ...
    const daySales = sales.filter(s => new Date(s.date).toLocaleDateString() === d.toLocaleDateString());
    const dayExpenses = expenses.filter(e => new Date(e.date).toLocaleDateString() === d.toLocaleDateString());
  ...
  const totalIncome = totalProfit - totalExpense;
  const totalDue = sales.reduce((acc, s) => acc + (s.dueAmount || 0), 0);
  
  const stockValue = products.reduce((acc, p) => {
    const qty = p.variants?.reduce((vAcc, v) => vAcc + (v.quantity || 0), 0) || 0;
  ...
  return (
    <div className="space-y-8 pb-10">
      <div className="flex flex-col gap-1 px-2">
        ...
        <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-2">{t.sub}</p>
      </div>
      ...
      <section className="bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] border border-white dark:border-slate-800 shadow-[0_15px_30px_rgba(0,0,0,0.03)] overflow-hidden relative">
        ...
        <div className="h-[320px] w-full relative">
          {hasData ? (
            <ResponsiveContainer width="100%" height="100%" key={`bar-${language}`}>
              <BarChart data={snapshotData} margin={{ top: 20, right: 10, left: 10, bottom: 20 }}>
                ...
                <Tooltip 
                  ...
                  contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', fontSize: '12px', fontWeight: 'bold', backgroundColor: '#1e293b', color: '#fff' }}
                  itemStyle={{ color: '#fff' }}
      ...
      <div className="grid grid-cols-3 gap-3 px-2">
        <div className="bg-white dark:bg-slate-900 p-4 rounded-3xl border border-slate-50 dark:border-slate-800 shadow-sm text-center">
          <p className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Profit</p>
          <p className="text-sm font-black text-emerald-600 dark:text-emerald-400">৳{totalProfit.toLocaleString()}</p>
        </div>
        <div className="bg-white dark:bg-slate-900 p-4 rounded-3xl border border-slate-50 dark:border-slate-800 shadow-sm text-center">
          <p className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Expense</p>
          <p className="text-sm font-black text-rose-500">৳{totalExpense.toLocaleString()}</p>
        </div>
        <div className="bg-white dark:bg-slate-900 p-4 rounded-3xl border border-slate-50 dark:border-slate-800 shadow-sm text-center">
          <p className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Income</p>
          <p className="text-sm font-black text-indigo-600 dark:text-indigo-400">৳{totalIncome.toLocaleString()}</p>
        </div>
      </div>
      ...
      <section className="bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] border border-white dark:border-slate-800 shadow-[0_15px_30px_rgba(0,0,0,0.03)] overflow-hidden">
        <h3 className="font-black text-slate-900 dark:text-white text-sm mb-6 flex items-center justify-between">
          {t.weekly}
          <span className="text-[9px] bg-slate-50 dark:bg-slate-800 px-3 py-1 rounded-full text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest">{t.view}</span>
        </h3>
        <div className="h-64 w-full relative">
          <ResponsiveContainer width="100%" height="100%" key={`area-${language}`}>
            <AreaChart data={last7Days}>
              ...
              <Tooltip 
                contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)', backgroundColor: '#1e293b', color: '#fff' }}
                itemStyle={{ color: '#fff' }}
              />
        ...
        <div className="flex items-center justify-center gap-6 mt-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-widest">Profit</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 bg-rose-500 rounded-full"></div>
            <span className="text-[10px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-widest">Expenses</span>
          </div>
        </div>
      </section>
    </div>
