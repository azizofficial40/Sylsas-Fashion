    ...
    <div className="fixed inset-0 bg-white z-[200] flex items-center justify-center p-6">
      <div className="w-full max-w-sm space-y-12 text-center">
        <div className="space-y-4">
          <div className="w-24 h-24 bg-indigo-600 rounded-[2.5rem] flex items-center justify-center text-white mx-auto shadow-2xl shadow-indigo-200 rotate-6 hover:rotate-0 transition-transform duration-500">
            <LogIn size={44} />
          </div>
          <h2 className="text-4xl font-black text-slate-900 tracking-tight">{t.loginTitle}</h2>
          <p className="text-slate-400 font-bold text-sm tracking-widest uppercase">{t.loginSub}</p>
        </div>
        ...
        <div className="space-y-6">
          <input 
            ...
            placeholder="••••"
            maxLength={4}
            className={`w-full py-8 bg-slate-50 border-0 rounded-[2rem] text-center text-4xl font-black tracking-[1em] outline-none transition-all focus:bg-white focus:ring-4 focus:ring-indigo-100 ${err ? 'bg-rose-50 text-rose-500' : 'text-slate-900'}`}
            value={pin}
          ...
          <button 
            onClick={handleLogin}
            className="w-full py-5 bg-indigo-600 text-white rounded-[1.5rem] font-black text-lg shadow-xl shadow-indigo-100 active:scale-95 transition-all"
          >
            {t.loginBtn}
          </button>
        </div>
...
const Navigation: React.FC<{ activeTab: TabType, setActiveTab: (tab: TabType) => void }> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <nav className="fixed bottom-6 left-4 right-4 glass-panel rounded-[2rem] flex justify-around items-center px-4 py-3 z-50 shadow-[0_15px_40px_rgba(0,0,0,0.1)] border border-white/40">
      {tabs.map((tab) => {
        const Icon = tab.icon;
        ...
        return (
          ...
          >
            <div className={`p-2 rounded-xl transition-all ${isActive ? 'bg-indigo-50 active-tab-glow' : ''}`}>
...
const LandingPage: React.FC<{ onGetStarted: () => void }> = ({ onGetStarted }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-50 z-[300] overflow-y-auto selection:bg-indigo-100">
      <nav className="px-6 py-8 flex items-center justify-between max-w-5xl mx-auto">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-indigo-200">
            <ShoppingBag size={24} />
          </div>
          <span className="text-xl font-black text-slate-900 tracking-tight">Sylsas</span>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className="px-6 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-black uppercase tracking-widest text-slate-900 shadow-sm hover:border-indigo-200 transition-all active:scale-95"
          >
            {isBn ? 'লগইন' : 'Login'}
          </button>
        </div>
      ...
      <main className="px-6 pt-16 pb-32 max-w-5xl mx-auto text-center space-y-20">
        <div className="space-y-8">
          <div className="inline-block px-4 py-1.5 bg-indigo-50 text-indigo-600 rounded-full text-[10px] font-black uppercase tracking-[0.2em] animate-bounce">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
            <button 
              onClick={onGetStarted}
              className="w-full sm:w-auto px-10 py-5 bg-indigo-600 text-white rounded-2xl font-black text-lg shadow-2xl shadow-indigo-200 hover:bg-indigo-700 transition-all active:scale-95"
            >
            ...
            <div className="flex -space-x-3">
              ...
              <div className="w-10 h-10 rounded-full border-2 border-white bg-indigo-50 flex items-center justify-center text-[10px] font-black text-indigo-600">
                +50
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 text-left space-y-4 hover:shadow-xl transition-all group">
...
const AppContent: React.FC = () => {
  ...
  const { error, language, setLanguage, logout, isLoggedIn, admin } = useStore();
  const t = TRANSLATIONS[language];
  ...
  return (
    <div className="min-h-screen pb-40">
      ...
      <header className="px-6 pt-10 pb-6 flex items-center justify-between">
        ...
        >
          <div className="w-12 h-12 rounded-[1.2rem] bg-white border border-slate-100 flex items-center justify-center text-slate-800 shadow-sm overflow-hidden group-hover:border-indigo-200 group-hover:shadow-md transition-all">
            <img src={admin.image} className="w-full h-full object-cover" alt="Logo" />
          </div>
          <div>
            <h1 className="text-xl font-black text-slate-900 tracking-tight leading-none group-hover:text-indigo-600 transition-colors">{admin.name}</h1>
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className="w-10 h-10 bg-white text-slate-600 rounded-2xl flex items-center justify-center shadow-sm border border-white hover:border-indigo-100 active:scale-90 transition-all"
          >
            <Languages size={18} />
          </button>
          <button 
            onClick={() => setActiveTab('ai')}
            className={`w-10 h-10 rounded-2xl flex items-center justify-center shadow-sm border transition-all ${activeTab === 'ai' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-indigo-600 border-white'}`}
          >
            <Sparkles size={18} />
          </button>
          <button 
            onClick={logout}
            className="w-10 h-10 bg-rose-50 text-rose-500 rounded-2xl flex items-center justify-center shadow-sm border border-rose-100 active:scale-90 transition-all"
          >
            <LogOut size={18} />
          </button>
        </div>
