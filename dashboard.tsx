    ...
    <div className="space-y-10">
      ...
      <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-white dark:border-slate-800 shadow-[0_30px_60px_rgba(0,0,0,0.05)] text-center relative overflow-hidden group">
        ...
        <div className="relative z-10">
          <p className="text-indigo-600 dark:text-indigo-400 font-black text-[11px] uppercase tracking-[0.3em] mb-3">{t.capital}</p>
          <h2 className="text-6xl font-black text-slate-900 dark:text-white tracking-tighter mb-8">৳{realtimeIncome.toLocaleString()}</h2>
          ...
          <div className="grid grid-cols-2 gap-4 border-t border-slate-50 dark:border-slate-800 pt-8">
            <div className="text-left px-4">
              ...
              <p className="text-xl font-black text-slate-800 dark:text-slate-200">৳{todaySales.toLocaleString()}</p>
            </div>
            <div className="text-right px-4 border-l border-slate-50 dark:border-slate-800">
              ...
              <p className="text-xl font-black text-slate-800 dark:text-slate-200">৳{stockValue.toLocaleString()}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-8 px-2">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-black text-slate-900 dark:text-white text-lg">{t.recent}</h3>
          </div>
        ...
        {sales.length === 0 ? (
          <div className="bg-white/40 dark:bg-slate-900/40 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-[2.5rem] py-16 text-center">
            <Package size={40} className="text-slate-200 dark:text-slate-800 mx-auto mb-4" />
            <p className="text-slate-400 font-bold text-sm tracking-wide">{t.waiting}</p>
          </div>
        ) : (
          <div className="space-y-4">
            {sales.slice(0, 4).map(sale => (
              <div key={sale.id} className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-white dark:border-slate-800 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h4 className="font-black text-slate-900 dark:text-white">{sale.productName}</h4>
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      <span className="text-[9px] font-bold text-slate-300 dark:text-slate-600 tracking-tighter"><Clock size={10} className="inline mr-1" /> {new Date(sale.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <div className={`text-[8px] px-2 py-0.5 rounded-full font-black uppercase tracking-widest mt-2 inline-block ${
                    sale.paymentStatus === 'Full Paid' ? 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600' : 'bg-rose-50 dark:bg-rose-950/30 text-rose-500'
                  }`}>
